// src/pages/InscriptionsUniversites.jsx
...
import {
  FaUniversity, FaChevronRight, FaHome, FaSignInAlt, FaSearch,
  ...
  FaPalette, FaMicroscope, FaEdit, } from 'react-icons/fa';

/* ── Palette ──────────────────────────────────────────────────── */
const C = {
  navy: '#0B1F4A', blue: '#185FA5', green: '#1D9E75',
...
const DEBOUCHES_MAP = [
  {
    ...
    salaire: '800–2 500 USD/mois', duree: '3–5 ans',
  ...
  },
  {
    ...
    salaire: '700–3 500 USD/mois', duree: '5–7 ans',
  ...
  },
  {
    ...
    salaire: '600–2 500 USD/mois', duree: '3–5 ans',
  ...
  },
  {
    ...
    salaire: '700–3 000 USD/mois', duree: '3–5 ans',
  ...
  },
  {
    ...
    salaire: '500–2 000 USD/mois', duree: '3–5 ans',
  ...
  },
  {
    ...
    salaire: '800–4 000 USD/mois', duree: '3–5 ans',
  ...
  },
  {
    ...
    salaire: '800–2 500 USD/mois', duree: '5 ans',
  ...
  },
  {
    ...
    salaire: '400–1 500 USD/mois', duree: '3–5 ans',
...
function getDebouches(filiereNom = '') {
  ...
  return (
    DEBOUCHES_MAP.find(d => d.keys.some(k => nom.includes(k))) || {
...
/* ════════════════════════════════════════════════════════════════ */
export default function InscriptionsUniversites() {
  ...
  return (
    <div style={{ fontFamily: "'Inter','Segoe UI',sans-serif", background: 'var(--bg-secondary)', minHeight: '100vh' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 1000, background: C.navy, boxShadow: '0 2px 20px rgba(0,0,0,0.3)' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 62, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div className="iu-nav-links" style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <Link to="/" style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontSize: 13, display: 'flex', alignItems: 'center', gap: 5 }}>
              <FaHome /> Accueil
            </Link>
            <Link to="/universites-publiques" className="iu-nav-secondary" style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontSize: 13 }}>
              Universités
            </Link>
            <Link to="/orientation" className="iu-nav-secondary" style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontSize: 13 }}>
              Orientation
            </Link>
            <Link to="/login" style={{ padding: '8px 18px', background: C.green, color: 'white', borderRadius: 8, textDecoration: 'none', fontSize: 13, fontWeight: 700, marginLeft: 8, display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
      ...
      <section style={{ background: `linear-gradient(135deg, ${C.navy} 0%, #0D2B6B 50%, ${C.blue} 100%)`, padding: '56px 24px 48px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 700, margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(29,158,117,0.2)', border: '1px solid rgba(29,158,117,0.35)', borderRadius: 30, padding: '5px 14px', marginBottom: 20 }}>
            <FaEdit style={{ color: '#9FE1CB', fontSize: 12 }} />
            <span style={{ color: '#9FE1CB', fontSize: 13, fontWeight: 600 }}>Inscriptions ouvertes 2026</span>
          </div>
          <h1 style={{ color: 'white', fontSize: 'clamp(26px,4.5vw,48px)', fontWeight: 900, lineHeight: 1.15, marginBottom: 16 }}>
            Choisissez votre université<br />
            <span style={{ background: 'linear-gradient(90deg,#9FE1CB,#7EC8D8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              et votre filière
            </span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.78)', fontSize: 16, lineHeight: 1.7, marginBottom: 32 }}>
            Explorez les universités qui acceptent des inscriptions, leurs facultés, les filières disponibles
            et les <strong style={{ color: '#9FE1CB' }}>débouchés professionnels</strong> avant de postuler.
          </p>
      ...
      <section style={{ background: 'var(--bg-card)', padding: '22px 24px', borderBottom: '1px solid var(--border-color)' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
          {[
            { num: '1', label: "Choisir l'université", active: true },
            { num: '2', label: 'Sélectionner la filière', active: true },
            { num: '3', label: 'Remplir le dossier', active: false },
            { num: '4', label: 'Inscription validée', active: false },
          ].map((step, i) => (
            <div key={step.num} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 18px' }}>
                <div style={{
                  width: 34, height: 34, borderRadius: '50%',
                  background: step.active ? C.navy : 'var(--border-color)',
                ...
                <span style={{ fontSize: 13, fontWeight: 600, color: step.active ? 'var(--text-primary)' : 'var(--text-muted)', whiteSpace: 'nowrap' }}>
                  {step.label}
                </span>
              </div>
              {i < 3 && <FaChevronRight style={{ color: 'var(--border-color)', fontSize: 11, flexShrink: 0 }} />}
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 20px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h2 style={{ color: 'var(--text-primary)', fontSize: 20, fontWeight: 800, margin: 0 }}>
              {uniFiltered.length} université{uniFiltered.length > 1 ? 's' : ''}
              {recherche ? ` pour "${recherche}"` : ' acceptent les inscriptions'}
            </h2>
            <p style={{ color: 'var(--text-muted)', fontSize: 13, margin: '4px 0 0' }}>
              Cliquez sur une université pour explorer ses facultés et filières
            </p>
          </div>
          {selectedUni && (
            <button
              onClick={() => { setSelectedUni(null); setSelectedDept(null); setFilieres([]); setDepartements([]); }}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 8, cursor: 'pointer', fontSize: 13, color: 'var(--text-secondary)' }}
            >
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--bg-card)', borderRadius: 16, color: 'var(--text-muted)' }}>
            ...
            <p style={{ margin: 0 }}>
              {recherche
                ? `Aucune université trouvée pour "${recherche}"`
                : "Aucune université n'accepte les inscriptions pour le moment."}
            </p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 16 }}>
            {uniFiltered.map(uni => (
              <UniCard key={uni.id} uni={uni} selected={selectedUni?.id === uni.id} onClick={handleUniClick} />
            ))}
          </div>
        )}
      </section>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20, fontSize: 13, color: 'var(--text-muted)', flexWrap: 'wrap' }}>
            ...
            {selectedDept && (
              <>
                ...
                <span style={{ color: C.blue, fontWeight: 600 }}>{selectedDept.nom}</span>
              </>
            )}
          </div>

          <div className="iu-explore-grid" style={{ display: 'grid', gridTemplateColumns: selectedDept ? '1fr 1fr' : '1fr', gap: 20 }}>
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: 28, boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{selectedUni.nom} — sélectionnez un département</div>
                </div>
              </div>
              ...
              {loadingDept ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[...Array(4)].map((_, i) => (
                    <div key={i} style={{ height: 52, borderRadius: 10, background: 'var(--bg-secondary)', animation: 'pulse 1.5s infinite' }} />
                  ))}
                </div>
              ...
              <div style={{ background: 'var(--bg-card)', borderRadius: 20, padding: 28, boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{selectedDept.nom} — cliquez "Info" puis "S'inscrire"</div>
                  </div>
                </div>
                ...
                {loadingFil ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {[...Array(3)].map((_, i) => (
                      <div key={i} style={{ height: 72, borderRadius: 12, background: 'var(--bg-secondary)', animation: 'pulse 1.5s infinite' }} />
                    ))}
                  </div>
                ) : filieres.length === 0 ? (
                  <EmptyState icon={<FaGraduationCap />} text="Aucune filière disponible pour ce département." />
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 520, overflowY: 'auto', paddingRight: 4 }}>
                    {filieres.map(filiere => (
                      <FiliereCard
                        key={filiere.id}
                        filiere={filiere}
                        universiteId={selectedUni.id}
                        departementId={selectedDept.id}
                        onInfo={setInfoFiliere}
                      />
                    ))}
                  </div>
                )}
      ...
      <section style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.blue})`, margin: '48px 0 0', padding: '52px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 560, margin: '0 auto' }}>
          ...
          <h2 style={{ color: 'white', fontSize: 28, fontWeight: 900, marginBottom: 12 }}>Vous avez fait votre choix ?</h2>
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 15, lineHeight: 1.7, marginBottom: 28 }}>
            Sélectionnez une filière ci-dessus et cliquez sur "S'inscrire" pour démarrer votre dossier en ligne.
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <Link to="/login" style={{ padding: '13px 28px', background: C.green, color: 'white', borderRadius: 10, textDecoration: 'none', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
              <FaSignInAlt /> J'ai déjà un compte
            </Link>
          </div>
        </div>
      </section>
      ...
      {infoFiliere && (
        <FilieresInfoModal
          ...
          universiteId={infoFiliere.universiteId}
          departementId={infoFiliere.departementId}
          onClose={() => setInfoFiliere(null)}
        />
      )}

      <style>{`
        @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.5} }
        .iu-search-input::placeholder { color: var(--text-muted); }
        @media (max-width: 640px) {
          .iu-nav-secondary { display: none !important; }
        }
        @media (max-width: 900px) {
          .iu-explore-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </div>
  );
...
function UniCard({ uni, selected, onClick }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(29,158,117,0.12)', color: '#1D9E75', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: 14, lineHeight: 1.3, marginBottom: 3, paddingRight: 52 }}>{uni.nom}</div>
...
function FiliereCard({ filiere, universiteId, departementId, onInfo }) {
  const deb = getDebouches(filiere.nom);
  return (
    <div
      style={{ padding: '14px 16px', borderRadius: 14, background: 'var(--bg-secondary)', border: '1.5px solid var(--border-color)', transition: 'all 0.18s' }}
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: 14, marginBottom: 5, lineHeight: 1.3 }}>{filiere.nom}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 11, color: '#1D9E75', fontWeight: 600 }}>{deb.salaire}</span>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 7, flexShrink: 0, alignItems: 'center' }}>
          <button
            onClick={() => onInfo({ filiere, universiteId, departementId })}
            style={{
          ...
          <Link
            to={`/inscriptions?universiteId=${universiteId}&departementId=${departementId}&filiereId=${filiere.id}`}
            style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', background: C.green, color: 'white', borderRadius: 20, textDecoration: 'none', fontSize: 12, fontWeight: 700 }}
          >
            S'inscrire <FaArrowRight style={{ fontSize: 9 }} />
          </Link>
        </div>
      </div>
    </div>
...
function FilieresInfoModal({ filiere, universiteId, departementId, onClose }) {
  const deb = getDebouches(filiere.nom);
  ...
  return (
    <div
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, background: 'rgba(11,31,74,0.65)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(4px)' }}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{ background: 'var(--bg-card)', borderRadius: 24, maxWidth: 640, width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.3)' }}
      >
        {/* En-tête coloré */}
        <div style={{ background: `linear-gradient(135deg, ${deb.color}, ${deb.color}bb)`, padding: '28px 28px 22px', position: 'relative', color: 'white', borderRadius: '24px 24px 0 0' }}>
          <button
            onClick={onClose}
            style={{ position: 'absolute', top: 18, right: 18, background: 'rgba(255,255,255,0.2)', border: 'none', color: 'white', width: 32, height: 32, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}
          >
            <FaTimes />
          </button>
          ...
          <div style={{ display: 'flex', gap: 16, fontSize: 13, opacity: 0.9, flexWrap: 'wrap' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <FaClock style={{ fontSize: 11 }} /> Durée : {filiere.duree || deb.duree}
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <FaBriefcase style={{ fontSize: 11 }} /> {deb.label}
            </span>
          </div>
        </div>
        ...
        <div style={{ padding: 28 }}>
          {/* Description */}
          {filiere.description && (
            <p style={{ color: 'var(--text-secondary)', fontSize: 14, lineHeight: 1.7, marginBottom: 24 }}>{filiere.description}</p>
          )}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <Link
              to={`/inscriptions?universiteId=${universiteId}&departementId=${departementId}&filiereId=${filiere.id}`}
              onClick={onClose}
              style={{ flex: 1, padding: '13px', background: C.green, color: 'white', borderRadius: 12, textDecoration: 'none', fontSize: 14, fontWeight: 700, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
            >
              <FaEdit /> S'inscrire dans cette filière
            </Link>
  ...
  return (
    <div style={{ background: 'var(--bg-card)', borderRadius: 18, padding: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
      <div style={{ display: 'flex', gap: 14, marginBottom: 14 }}>
        <div style={{ width: 58, height: 58, borderRadius: 14, background: 'var(--bg-secondary)', animation: 'pulse 1.5s infinite' }} />
        <div style={{ flex: 1 }}>
          <div style={{ height: 14, background: 'var(--bg-secondary)', borderRadius: 6, marginBottom: 8, animation: 'pulse 1.5s infinite' }} />
          <div style={{ height: 11, background: 'var(--bg-secondary)', borderRadius: 6, width: '60%', animation: 'pulse 1.5s infinite' }} />
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 14 }}>
        {[...Array(3)].map((_, i) => (
          <div key={i} style={{ height: 44, background: 'var(--bg-secondary)', borderRadius: 10, animation: 'pulse 1.5s infinite' }} />
        ))}
      </div>
      <div style={{ height: 24, background: 'var(--bg-secondary)', borderRadius: 20, width: '40%', animation: 'pulse 1.5s infinite' }} />
    </div>
  );
}
